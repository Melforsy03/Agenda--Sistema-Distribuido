...
def show_create_event_view(user_id):
    ...
    if is_group_event:
        ...
        if groups:
            group_name = st.selectbox("Selecciona grupo", [g[1] for g in groups])
            group_id = [g[0] for g in groups if g[1] == group_name][0]
    ...
    if st.button("Crear evento"):
        ...
        event_id, error = EventService().create_event(
            title, description, start_str, end_str,
            user_id, group_id, is_group_event, participants_ids
        )
        if event_id:
