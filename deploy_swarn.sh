...
if [[ "$NODE_ROLE" == "manager" ]]; then
  ...
  # === Frontend en el manager (útil cuando no hay workers) ===
  if ! docker ps --format '{{.Names}}' | grep -q '^frontend$'; then
    log "🎨 Iniciando frontend en el manager (para lab/demo sin workers)..."
    docker run -d \
      --name frontend \
      --hostname frontend \
      --network $NETWORK_NAME \
      ...
      -e WEBSOCKET_PORT=${BACKEND_PORT_WS} \
      $FRONTEND_IMAGE \
      streamlit run front/app.py --server.port=${FRONTEND_PORT} --server.address=0.0.0.0
  else
    log "ℹ️  Frontend ya está corriendo en este nodo."
  fi
